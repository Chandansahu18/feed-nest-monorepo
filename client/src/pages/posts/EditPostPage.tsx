...
const EditPostPage = () => {
  ...
  const [postData, setPostData] = useState<TPostDataUpdate>({
    postTitle: "",
    postDescription: "",
    postBannerImage: "",
    postTags: [],
  ...
  const { data: existingPost, isPending: isPostLoading } = usePostData(postId);
  const { mutate: updatePost, isPending: isUpdating } = usePostDataUpdate();
  const { mutate: enhanceContent } = useEnhanceContent();

  useEffect(() => {
    if (existingPost) {
      setPostData({
        postTitle: existingPost.data?.postTitle,
        postDescription: existingPost.data?.postDescription || "",
        postBannerImage: existingPost.data?.postBannerImage || "",
        postTags: existingPost.data?.postTags || [],
        published: existingPost.data?.published || false,
      });
    }
  }, [existingPost]);

  const handleInputChange = (
    field: keyof TPostDataUpdate,
  ...
  // Fix: Add function to handle banner image removal
  ...
  const handleUpdatePost = () => {
    ...
    updatePost(
      { postId, updatedPostData: postData },
      {
        onSuccess: () => {
          setShowPreview(true);
        },
              ...
              <Card className="bg-card border-0 shadow-none lg:border lg:shadow-sm rounded-2xl">
                <CardContent className="p-4 sm:p-6 lg:p-8 xl:p-12">
                  ...
                  <h1 className="text-xl sm:text-2xl lg:text-3xl xl:text-4xl font-bold mb-4 sm:mb-6 leading-tight">
                    {postData?.postTitle || "Untitled Post"}
                  </h1>
                  ...
                  {postData?.postDescription && (
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html:
                          typeof postData.postDescription === "string"
                            ? postData.postDescription
                            : String(postData.postDescription || ""),
                      }}
                    />
                  )}
