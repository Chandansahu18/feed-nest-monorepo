...
import handleGetAuthenticatedUser from "../controllers/user/handleGetAuthenticatedUser.controller";

const router = express.Router();

router.route("/v1/user").get(handleGetUser).get(handleGetAuthenticatedUser).patch(handleUserDetailsUpdate).delete(handleUserAccountDelete);
